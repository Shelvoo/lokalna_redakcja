...
for (let i = 0; i < 10; i++) {
  var changeable = posts[i];
  if (changeable.featured == true) {
    var boks = document.createElement("div")
    boks.classList.add("box")
    boks.setAttribute("id", "box"+i)
    boks.textContent = posts[i].title
    var specjal = document.getElementById("ts")
    specjal.appendChild(boks)
    console.log("Works if" + i)
  }
  else if (posts[i].date > "2025-10-08T10:00:00+02:00") {
    var boks = document.createElement("div")
    boks.classList.add("box")
    boks.setAttribute("id", "box"+i)
    boks.textContent = posts[i].title
    var time = document.getElementById("tl")
    time.appendChild(boks)
    console.log("Works else" + i)
  }
  else {
    var boks = document.createElement("div")
    boks.classList.add("box")
    boks.setAttribute("id", "box"+i)
    boks.textContent = posts[i].title
    var art = document.getElementById("ta")
    art.appendChild(boks)
    console.log("Works elsen" + i)
  }
}
for (let i = 0; i < 10; i++) {
    var cover = document.createElement("img")
    cover.url = posts[i].cover
    var boxid = document.getElementById("box"+i)
    boxid.appendChild(cover)
}